...
risk_level: high
last_verified_commit: dffb8046c2adf92b86d092ecaa7a459e2a9ca811
updated: 2026-08-06
---
...
| 自动化验证 | 暂无 CI 和 lint，外部服务不能由 Mock 代替 | 已加入 Node 原生测试入口；60 项默认测试、12/12 macOS 无写入探针、语法、差异、严格状态校验和隔离端口健康检查通过 | 用户真实验收后把结果回写本文件；后续版本再评估 CI |
...
- 当前状态：60 项默认自动化测试及 12/12 macOS 无写入 PTY 探针通过；修复后服务已在隔离端口重新启动；真实登录钥匙串保存等待用户重试；默认端口 3210 仍由未知进程占用且未处理。
...
| Key 不进入仓库、普通配置、日志或进程参数 | 测试断言假 Key 只经受控 PTY 传入且不在 argv/环境/输出/错误/响应；源码无浏览器持久化；差异检查通过 | 真实运行后检查 Git 状态与应用日志；不向 AI 提供 Key | 自动通过，真实待验收 |
...
| 验收可重复 | `npm test` 60/60；macOS 无写入 PTY 探针 12/12；新文件语法、差异、隔离端口健康、桌面/窄屏布局与严格状态校验通过 | 按设计稿 §12.2 完成人工验收并回写 | 无 Key 部分通过 |
...
用户刷新 `http://localhost:43211/`，只在护航面板重新输入自己的 Key，先确认修复后的“保存并检测”是否成功；成功后继续“真实对话 → 替换为无效 Key 并识别错误 → 恢复有效 Key → 删除 → Pi 不可用时仍能护航”，全程不把 Key 交给 AI。
...
- 已完成：独立 DeepSeek Provider、修订后的 macOS Keychain PTY 适配器、护航状态机、认证路由、常驻/响应式护航面板，以及 60 项默认测试、macOS 无写入探针和本地页面验证。
